...
const ToggleBackground = styled.div`
  height: 100%;
  width: 100%;
  transition: ${props => props.theme.transition({ t: ['all'], d: '.4' })};
...
const ToggleIndicator = styled.div`
  ...
  ::before {
    ...
    height: 100%;
    border-radius: 50%;
    transition: ${props => props.theme.transition({ t: ['opacity'], d: '.4' })};
    opacity: 0;
    background: ${props => props.theme.mainGradient};
  }
  ...
  if (props.label) {
    return (
      <InputLabel label={props.label}>
        <Checkbox type='checkbox' defaultChecked={!!props.checked} />
        <ToggleContainer>
          <ToggleBackground />
          <ToggleIndicator />
        </ToggleContainer>
